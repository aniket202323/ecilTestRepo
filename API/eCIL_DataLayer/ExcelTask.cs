...
namespace eCIL_DataLayer
{
    public class ExcelTask
    {
                ...
                using (SqlConnection conn = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            //BugFix 10798 Autopone Task were showing in Modify status even with no changes done.
                            if (!reader.IsDBNull(reader.GetOrdinal("FixedFrequency")))
                                temp.FixedFrequency = reader.GetString(reader.GetOrdinal("FixedFrequency"));

                            //Commented the below to Fix Bug 10798 Autopone Task were showing in Modify status even with no changes done.
                            //if (!reader.IsDBNull(reader.GetOrdinal("FixedFrequency")))
        ...
        public string CompareRawDataAndProficy(string _connectionString, bool moduleLevelComparision, string path, string sheet, int plId, int puId)
        {
            ...
            var plantmodeldata = new List<PlantModel.PlantModelData>();
            var plantmodellinedata = new List<PlantModel.ProficyDataSource>();
            string RawDataFL3;
            ...
            if (moduleLevelComparision == true)
            {
                ...
                plantmodeldata = GetModuleHierarchyInfo(_connectionString, puId);
                plantmodellinedata = GetTaskListFromProductionLine(_connectionString, plId.ToString());
                ...
                {
                    for (int j = 0; j < plantmodellinedata.Count; j++)
                    {

                        if ((rawDatataskslist[i].ECIL_VMID.Equals(plantmodellinedata[j].VMId)) && (!(rawDatataskslist[i].eCIL_Module.Equals(plantmodellinedata[i].SlaveUnitDesc))))
                        {

                            errormessage = "Task with same VM ID found on a different module on the line";
                            return errormessage;
                        }
                    }
                }
