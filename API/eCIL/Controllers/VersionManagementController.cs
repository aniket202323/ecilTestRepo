        ...
        public string ReadProficyData(string path, bool lineLevelComparision, bool moduleLeveComparision, string plId, string puId)
        {
            ...
            if (_UserRepository.CheckJwtToken(jwtToken) == 4)
            {
                ...
                try
                {
                    ...
                    //BugFix 10743 Check if duplicate VM ID is saved across modules when task is created by  module level comparison
                    // Get the list of tasks for entire line irrespective of line level or module level comparision to find duplicate VMIDs accross the line
                    ...
                    result = et.GetTaskListFromProductionLine(ConfigurationManager.ConnectionStrings["ServerName"].ConnectionString + ConfigurationManager.AppSettings["DatabaseConnection"], plId);
